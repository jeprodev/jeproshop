...
class JeproshopConnectionModelConnection extends JModelLegacy
{
    /** @var integer */
    public $guest_id;
...
class JeproshopConnectionSourceModelConnectionSource extends JModelLegacy
{
	...
	public static function getOrderSources($order_id){
		$db = JFactory::getDBO();
		
		$query = "SELECT connection_source.http_referrer, connection_source.request_uri, connection_source.keywords, connection_source.date_add FROM " . $db->quoteName('#__jeproshop_orders') . " AS ord INNER JOIN ";
		$query .= $db->quoteName('#__jeproshop_guest') . " AS guest ON guest.customer_id = ord.customer_id INNER JOIN " . $db->quoteName('#__jeproshop_connection') . " AS connection  ON connection.guest_id = guest.guest_id ";
		$query .= " INNER JOIN " . $db->quoteName('#__jeproshop_connection_source') . " AS connection_source ON connection_source.connection_id = connection.connection_id WHERE order_id = " . (int)($order_id) . " ORDER BY connection_source.date_add DESC";
