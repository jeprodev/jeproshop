...
class JeproshopShopModelShop extends JModelLegacy
{
	...
	public function __construct($shop_id = null, $lang_id = null){
		...
		if($shop_id){
			...
			if(!JeproshopCache::isStored($cache_id)){
				...
				$db->setQuery($query);
				$shop_data = $db->loadObject(); //print_r($shop_data);
				if($shop_data){
	...
	public static function initialize(){
		...
		if(!$shop_id){ 
			...
			if($throuth && $shop_id &&!$is_main_uri){
				foreach ($results as $result){
	...
	public static function getCompleteListOfShopsId(){
		$db = JFactory::getDBO();
		$list = array();
		$query = "SELECT shop_id FROM " . $db->quoteName('#__jeproshop_shop');
	
		$db->setQuery($query);
		$result = $db->loadObjectList();
		foreach ($result as $row){ $list[] = $row->shop_id; }
	...
	*/
    ...
    public static function getContextShopGroup(){
        ...
        if ($context_shop_group === null)
            $context_shop_group = new JeproshopShopGroupModelShopGroup((int)self::$context_shop_group_id);
