...
<form action="<?php echo JRoute::_('index.php?option=com_jeproshop&view=tax'); ?>" method="post" name="adminForm" id="adminForm" class="form-horizontal" >
    ...
    <div id="j-main-container"  <?php if(!empty($this->sideBar)){ echo 'class="span10"'; }?> >
        <div class="box_wrapper jeproshop_sub_menu_wrapper" ><?php echo $this->renderSubMenu('tax'); ?></div>
		<table class="table table-striped" id="stateList" >
			<thead>
                <tr>
                    ...
                    <th width="4%" class="nowrap  " ><?php echo JText::_('JSTATUS'); ?></th>
                    <th width="60%" class="nowrap " ><?php echo JText::_('COM_JEPROSHOP_TAX_NAME_LABEL'); ?></th>
                    <th width="5%" class="nowrap " ><?php echo JText::_('COM_JEPROSHOP_TAX_RATE_LABEL'); ?></th>
            ...
            <tbody>
                <?php if(isset($this->taxes)){
                    foreach($this->taxes as $index => $tax){
                        $taxLink = JRoute::_('index.php?option=com_jeproshop&view=tax&task=edit&tax_id=' . (int)$tax->tax_id . '&' .JeproshopTools::getTaxToken() . '=1', true, 1);
                        $deleteTaxLink = JRoute::_('index.php?option=com_jeproshop&view=tax&task=delete&tax_id=' . (int)$tax->tax_id . '&' .JeproshopTools::getTaxToken() . '=1', true, 1);
                        ?>
                <tr class="row_<?php echo ($index%2); ?>" >
                    ...
                    <td width="4%" class="nowrap"><i class="<?php echo 'icon-' . ($tax->published ? '' : 'un' ) . 'publish' ?>" ></i></td>
                    <td width="60%" class="nowrap"><a href="<?php echo $taxLink; ?>" ><?php echo ucfirst($tax->name); ?></a> </td>
                    <td width="5%" class="nowrap center"><?php echo $tax->rate; ?></td>
